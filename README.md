...
**UniDepth** (used for SUNRGBD / Hypersim / ARKitScenes / Objectron / KITTI / nuScenes) — set up a separate env per [UniDepth's official instructions](https://github.com/lpiccinelli-eth/UniDepth), then:
```bash
python tools/unidepth_script.py --dataset SUNRGBD --split train
# repeat for each (dataset, split) you need
```

**Metric3D** (used for Cityscapes3D) — set up a separate env per [Metric3D's official instructions](https://github.com/YvanYin/Metric3D), then:
```bash
