...
def do_test(cfg, model, iteration='final', storage=None, mode="novel"):
    ...
    if mode == "novel":
        dataset_names_test = cfg.DATASETS.TEST_NOVEL
        eval_categories = cfg.DATASETS.CATEGORY_NAMES_NOVEL
    elif mode == "base":
        dataset_names_test = cfg.DATASETS.TEST_BASE
        eval_categories = cfg.DATASETS.CATEGORY_NAMES_BASE
    else:
...
def setup_training_dataset(cfg, filter_settings):
    ...
    del datasets
    gc.collect() 
    return dataset_id_to_unknown_cats, dataset_id_to_src, priors
