    ...
    def forward(self, images, features, proposals, Ks, im_scales_ratio, targets=None, prompt_depth=None, **kwargs):
        ...
        if self.training:
            ...
            if isinstance(proposals, list) and ~np.any([isinstance(p, Instances) for p in proposals]):
                pred_instances = []
                for proposal, im_dim, im_scale_ratio in zip(proposals, im_dims, im_scales_ratio):
                    
                    pred_instances_i = Instances(im_dim)
                    pred_instances_i.pred_boxes = Boxes(proposal['gt_bbox2D'] / im_scale_ratio).to(device=list(features.values())[0].device)
                    pred_instances_i.pred_classes =  proposal['gt_classes'].to(device=list(features.values())[0].device)
                    if "gt_scores" in proposal.keys():
                        pred_instances_i.scores = proposal['gt_scores'].to(device=list(features.values())[0].device)
                    else:
                        pred_instances_i.scores = torch.ones_like(proposal['gt_classes']).float().to(device=list(features.values())[0].device)
    ...
    def _forward_cube(self, features, instances, Ks, im_current_dims, im_scales_ratio, prompt_depth=None):
        ...
        if cube_z is not None and self.cluster_bins > 1:
            ...
            if self.dims_priors_enabled:
                ...
                cube_z = cube_z[fg_inds, :, :][fg_inds, assignments[fg_inds]]
        ...
        if self.dims_priors_enabled:
            ...
            cube_dims = cube_dims[fg_inds, :]
            cube_pose = cube_pose[fg_inds, :, :]
        ...
        if self.use_confidence:
            ...
            if self.dims_priors_enabled:
                ...
                cube_uncert = cube_uncert[fg_inds,0]
        if self.dims_priors_enabled:
            cube_2d_deltas = cube_2d_deltas[fg_inds, box_classes, :]
        else:
            cube_2d_deltas = cube_2d_deltas[fg_inds, :]
        ...
        if self.dims_priors_enabled:
            ...
            if self.dims_priors_enabled:
                ...
                prior_dims = self.priors_dims_per_cat.detach().mean(dim=1).repeat([n, 1, 1])[fg_inds]
        ...
        elif self.z_type == 'clusters': # not yet change to class agnostic
