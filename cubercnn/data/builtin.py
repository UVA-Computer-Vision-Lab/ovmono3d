...
def get_omni3d_categories(dataset="omni3d"):
    ...
    if dataset == "omni3d":
        ...
        assert len(cats) == 9
    elif dataset in [ "SUNRGBD_test_novel"]:
        cats = set({'monitor', 'bag', 'dresser', 'board', 'printer', 'keyboard', 'painting', 'drawers', 'microwave', 'computer', 'kitchen pan', 'potted plant', 'tissues', 'rack', 'tray', 'toys', 'phone', 'podium', 'cart', 'soundsystem'})
        assert len(cats) == 20
    elif dataset in [ "ARKitScenes_test_novel"]:
        cats = set({'fireplace'})
        assert len(cats) == 1
